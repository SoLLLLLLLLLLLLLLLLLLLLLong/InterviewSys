...
export class LAppLive2DManager {
  ...
  public onTap(x: number, y: number): void {
    ...
    console.log('🎯 [LipSync] onTap triggered - Starting audio process');

    for (let i = 0; i < this._models.getSize(); i++) {
      // Use public folder path for Vite - files in public/ are served from root
      let filePath = "/audio/sample.wav";  // WAV file works with Three.js AudioAnalyser

      console.log(`🎵 [LipSync] Loading audio file: ${filePath}`);
      console.log(`🤖 [LipSync] Model index: ${i} / ${this._models.getSize()}`);
