...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // リップシンクの設定 (LAST - Highest Priority)
    // Applied after all other parameter updates to ensure lip sync overrides everything
    if (this._lipsync) {
      let value = 0.0; // リアルタイムでリップシンクを行う場合、システムから音量を取得して、0~1の範囲で値を入力します。

      // Using GenericAudioFileHandler for multi-format audio support
      this._genericAudioFileHandler.update();
      value = this._genericAudioFileHandler.getNormalizedAverageFrequency();

      // Use setParameterValueById instead of addParameterValueById to override all other animations
      // This ensures lip sync has the highest priority over expressions, motions, and other parameter updates
  ...
  _genericAudioFileHandler: GenericAudioFileHandler; // 汎用オーディオファイルハンドラ
